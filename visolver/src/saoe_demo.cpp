...
// ----------------------------------------------
// SAOE demo: random instance, solve the Nash equilibrium, print the tables.
// ----------------------------------------------
#include "saoe.hpp"
#include "utils.hpp"
...
// Nash equilibrium (saoe), and print the problem and solution using the shared
// table display in utils. The 'latexOutput' switch selects the format:
//   true  -> LaTeX tabular, ready to paste into a paper;
//   false -> plain, column-aligned ASCII.
int
main()
{
  ScopedUtcTimer timer("saoe_demo");
  ...
  const VIResult r = saoe(R, S);
  const SaoeSolution sol = saoeDecode(r, numActors, numOptions);
  ...
  std::snprintf(line, sizeof line, "SAOE: %d actors, %d options, seed %llu, eps = %.3e",
                ...
                saoeEps(R));
  printComment(latexOutput, line);
  saoePrintInputs(R, S, latexOutput);
  ...
  printComment(latexOutput, line);
  saoePrintSolution(R, sol.e, saoeEps(R), latexOutput);
