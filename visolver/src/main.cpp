...
// ----------------------------------------------
// Demo / regression: a monotone LCP solved as an LVI on K = R_+^2.
// ----------------------------------------------
#include "dhan06.hpp"
#include "utils.hpp"
...
main()
{
  ...
  const VIResult r =
      dHan06(x0, M, q, projectNonnegative,
             magTol, iterMax, iterFreq, DHan06Params{}, logger);

  const double solErr = (r.z - xStar).norm();
  printf("\nsolution       = (%.12f, %.12f)\n", r.z(0), r.z(1));
  printf("expected       = (%.12f, %.12f)\n", xStar(0), xStar(1));
  printSolveStats("dHan06", r);
