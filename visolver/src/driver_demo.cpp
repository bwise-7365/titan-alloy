...
// ----------------------------------------------
// Demo / regression for the outer Josephy-Newton driver on a nonlinear mixed VI.
// ----------------------------------------------
...
static void
printLinearForm(const char* name, const VectorXd& coeffs, Index n)
{
  printf("  %s =", name);
  bool any = false;
  for (Index j = 0; j < coeffs.size(); ++j) {
    const double a = coeffs(j);
    if (1.0e-12 < std::abs(a)) {
...
static void
printFieldComponent(const char* name, const MatrixXd& A, const VectorXd& k, Index row)
{
...
main()
{
  ScopedUtcTimer timer("lvi_vi_demo");
  ...
  makeComplementaryPair(m, rng, yLo, yHi, wStar, yStar);
  ...
  const InnerSolver innerSolver =
      makeDHan06Solver(innerMagTol, innerIterMax, 0);
  ...
  const VIResult r = solveVI(model, z0, innerSolver, params, logger);
