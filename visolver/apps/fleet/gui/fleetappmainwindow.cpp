...
// ----------------------------------------------
// FleetAppMainWindow implementation: a copy of the network fleet viewer window
// whose four solver call sites (optimal / greedy / swap / purify) route through
// the App::Fleet problem class. Everything else -- controls, layout, the
// per-asset slicing, the worker-thread + solve-token discipline, the status
// panel -- is verbatim, and the shared widgets come from viewer_common.
// ----------------------------------------------
...
namespace VIMCP::App {
  ...
  FleetAppMainWindow::FleetAppMainWindow(QWidget* parent)
    : PlannerGui(Texts{.windowTitle = "Fleet-plan app",
                       .nearestLinksTip =
                 ...
                 parent)
  {
    ...
    connect(fleetMultipleSlider_, &QSlider::valueChanged, this,
            &FleetAppMainWindow::onFleetMultipleMoved);
    connect(fleetMultipleSlider_, &QSlider::sliderReleased, this,
            &FleetAppMainWindow::regenerate);   // counts change the instance
    connect(assetBox_,
            QOverload<int>::of(&QSpinBox::valueChanged), this,
            &FleetAppMainWindow::applyDisplayedAsset);
    connect(swapOptButton_, &QPushButton::clicked, this,
            &FleetAppMainWindow::onSwapToOptimum);
    connect(purifyButton_, &QPushButton::clicked, this,
            &FleetAppMainWindow::onPurify);
    connect(resetButton_, &QPushButton::clicked, this,
            &FleetAppMainWindow::onResetPlan);
  ...
  FleetProfile
  FleetAppMainWindow::currentProfile() const
  {
  ...
  double
  FleetAppMainWindow::fleetMultiple() const
  {
  ...
  Index
  FleetAppMainWindow::displayedAsset() const
  {
    return static_cast<Index>(assetBox_->value() - 1);
  }

  void
  FleetAppMainWindow::rebuildInstance(std::uint64_t seed)
  {
    const FleetProfile profile = currentProfile();
    validateFleetProfile(profile);
    current_ = Fleet::generate(profile, seed);
  ...
  Index
  FleetAppMainWindow::nodeCount() const
  {
    return current_.numNodes;
  }

  void
  FleetAppMainWindow::refreshMap()
  {
  ...
  void
  FleetAppMainWindow::applyPlanMode()
  {
    if (!haveInstanceP_) {
      return;
    }
    ...
    if (greedyRadio_->isChecked()) {
      ...
      try {
        if (1 != planKind_) {
          ...
          if (!greedyValidP_) {
            greedy_ = Fleet(current_).greedyPlan();
            greedyValidP_ = true;
          }
  ...
  void
  FleetAppMainWindow::showWorkingPlan()
  {
  ...
  void
  FleetAppMainWindow::startOptimalSolve()
  {
    optimalBusyP_ = true;
    startBusy();
    refreshStatus("solving optimal fleet plan (ipm, screened + certified)...");

    // Everything the worker needs is captured BY VALUE; it touches no widget.
    // The solve goes through App::Fleet (which wraps solveFleetPlan). Budgets
    // are DATA (B_k = N_k v_k H), so there is no calibration pre-pass.
    ...
    optimalWatcher_->setFuture(QtConcurrent::run([inst, token]() mutable {
      ...
      out.token = token;
      try {
        out.result = std::get<1>(Fleet(inst).solve(FleetParams{}));
        out.okP = true;
      }
      catch (const std::exception& ex) {
  ...
  FleetAppMainWindow::onSwapToOptimum()
  {
    ...
    swapWatcher_->setFuture(QtConcurrent::run([inst, plan, token,
      ...
      out.kind = kind;
      try {
        out.summary = Fleet(inst).swapToLocalOptimum(plan);
  ...
  FleetAppMainWindow::onPurify()
  {
    ...
    purifyWatcher_->setFuture(QtConcurrent::run([inst, plan, token,
      ...
      out.kind = kind;
      try {
        out.summary = Fleet(inst).sparsify(plan);
        out.plan = std::move(plan);
        out.okP = true;
      }
      catch (const std::exception& ex) {
  ...
  void
  FleetAppMainWindow::refreshStatus(const QString& modeNote)
  {
  ...
  void
  FleetAppMainWindow::refreshPlanStatus()
  {
  ...
  void
  FleetAppMainWindow::refreshPlanControls()
  {
  ...
  QStringList
  FleetAppMainWindow::nodeInfoLines(int node) const
  {
