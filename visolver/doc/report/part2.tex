...
% ----------------------------------------------
% Part II -- Developer manual / API. First-drafted part (it pins terminology).
% Sources: the headers under include/ (each engine's contract comment is the
% authoritative statement), CLAUDE.md, CMakeLists.txt.
% ----------------------------------------------
...
vocabulary the rest of the report uses. Everything lives in the namespace
\code{VINCP}; all mathematical statements are deferred to
Chapter~\ref{ch:math} --- here they appear only as far as an API contract
needs them.
...
\code{FetchContent} at a pinned release tag and compiled by the same
...
\code{src/}                  & runnable demos (printout, not pass/fail gates) \\
...
elsewhere, and warnings are treated as errors in spirit; Eigen's headers are
included as \code{SYSTEM} and exempt.
...
\begin{itemize}
\item \textbf{\code{residual} is the squared Euclidean norm of the
  \emph{natural residual}} --- the standard defect vector of
  complementarity theory, which is zero exactly at solutions. On our $K$ it
  reads componentwise: $H(x,y)$ on the free rows and $\min(y_i, G_i)$ on
  the nonnegative rows, so it measures equation violation, infeasibility,
  and non-complementarity in one vector (the formal projection form and
  its properties are Section~\ref{sec:a-problem}). Every tolerance in the
  library (\code{magTol}, \code{outerTol}, \ldots) is likewise a
  \emph{squared} norm --- a deliberate, library-wide convention
  (Section~\ref{sec:contracts}).
\item \textbf{\code{converged = false} is an honest answer, not an error.}
  A solver that stalls --- a failed line search, a vanishing merit gradient,
  a collapsed interior-point step length, the Josephy--Newton no-progress
  ...
  \emph{best point visited} (in the natural-residual sense), not the last
  one, because their nonmonotone strategies may deliberately move above the
  best before stalling.
\end{itemize}
...
\item \code{makeEllipsoidProjector(radii, tol, iterMax)}
  ...
  sake and as the standing proof that the projection engines really are
  generic in $K$.
\end{itemize}

Not every engine honors an arbitrary $K$: the interior-point method and the
...
granularities. Loggers are the intended observability seam: production runs
stay silent, a long diagnostic run attaches a flushed per-iteration
heartbeat (a lesson recorded in Chapter~\ref{ch:testing}: under a piped test
runner, unflushed output from a killed run is simply lost).
...
uniform, cross-engine comparisons (and this report's tables) are
apples-to-apples. Do not ``fix'' a squared residual to a plain norm ---
halving the exponent of every published tolerance would silently change
...
\begin{itemize}
\item A \emph{bad value} --- NaN residual, divergence past the guard
...
\item A \emph{stall} --- no descent direction, a failed line search, an
  ...
  no-progress cutoff --- is an honest outcome at a real iterate: the solver
  returns it with \code{converged = false}. The distinction is not
  cosmetic: engine \emph{compositions} (Section~\ref{sec:altchain}) treat a
  stalled stage as ``switch strategy'' rather than ``abort,'' and a throw
  where a stall belongs forces every wrapper to catch and second-guess it.
\end{itemize}

\paragraph{One result type, one convention per seam.} Every solver returns
\code{VIResult}; every swappable component is a \code{std::function} seam
with a documented contract (\code{Projector}, \code{InnerSolver},
\code{NewtonSolverFactory}, \code{NcpFunctionPair}, \code{JacobianFn},
\code{StageSolver}). Seams are preferred over boolean flags wherever a third
implementation is plausible.

\section{Engine catalog}
\label{sec:engines}

The library deliberately offers a \emph{menu} of engines rather than one
...
belong (definitions in Section~\ref{sec:a-problem}). Each iteration factors $(I + \beta_k M)$ with a
self-adaptive $\beta_k$; \code{DHan06Params} exposes the relaxation factor
\code{gamma} $\in (0,2)$ (default 1.6), Han's constant \code{mu} (1.05), the
initial \code{beta0}, the $\tau$-schedule (\code{tau0}, \code{tauN}), the
divergence guard, and the diagnostic switch \code{adaptBeta}.
...
metric $(I + \beta_0 M) = (M + I)$ --- exactly \code{bsHe94b}'s
...
changes, the factorization is redone \emph{every iteration}: on equal
iteration counts \code{dHan06} has been observed an order of magnitude
slower in wall time than \code{bsHe94b}. With \code{adaptBeta = false} the
method \emph{is} \code{bsHe94b}, which isolates the self-adaptive rule for
A/B comparison.

\emph{Choose it} when the self-adaptive metric earns its cost --- in
practice, rarely; it is the reference implementation of its paper and the
comparison baseline.
...
direction) for the linear VI over any $K$. It factors $(M + I)$ \emph{once}
...
This is the library's workhorse for well-conditioned monotone affine
...
iteration count explodes --- the failure that motivated the interior-point
engine (Chapters~\ref{ch:testing} and \ref{ch:math}).
...
solution --- projection engines exploit such starts fully), and wherever
...
Its two distinctions: it is \textbf{matrix-free} ($M$ enters only through
products; nothing is factored), and it converges globally for merely
\textbf{pseudomonotone} continuous $F$ --- the weakest hypothesis of any
engine here. Its equally important limitation: whenever $\|F(x^\ast)\| > 0$,
the hyperplane step collapses like $\|r\|^2/\|F\|^2$ and the squared
residual decays only as $O(1/\sqrt{k})$. It is a \emph{globalizer, not a
finisher} --- good for getting from an arbitrary start into a solution's
neighborhood, too slow to polish an answer to tight tolerance --- and its
...
run to the caller's \code{magTol}. Phase 1 hitting its cap is \emph{not} an
error --- an honest warm start is the point. \code{ChainedSolverParams}
nests the two phases' params. Accounting follows the composite convention:
\code{iter} is phase 2's count, \code{innerIters} the total across both.
This chain is the small-scale ancestor of the model-level alternating chain
(Section~\ref{sec:altchain}): each method plays only its strong regime.
...
the \code{InnerSolver} seam.

Two documented safeguards go beyond the
2004 recipe: a conservative sampled-secant preflight for $\gamma_0$
(automatic when \code{gamma0} $\le 0$), and Tseng-style step backtracking,
which extends robustness to merely locally-Lipschitz fields; both are
self-erasing on gentle fields.

\emph{Choose it} for nonlinear monotone VIs where Jacobians are unwelcome
(black-box $F$, memory limits) or where a simple robust iterator matters
more than a fast tail --- its historical role: it solved the strategic
allocation game in the PME reference implementation, and its library port
independently confirms that game's reference equilibrium
...
\item \textbf{No start vector.} An interior-point method cannot use a caller
  ...
  starts from a data-scaled interior point. Consequently it \emph{cannot
  warm-start} --- relevant for re-solve loops.
\item \textbf{No \code{Projector}.} $K$ is structurally
  $\mathbb{R}^{\code{numFree}} \times \mathbb{R}^{m}_{+}$; pairing the seam
...
Its reason to exist: the iteration count ($\sim$10--40) is essentially
...
($10^{-10}$; a damped step below this returns honestly with
...
\paragraph{The \code{NewtonSolverFactory} seam.} The per-iteration linear
algebra is swappable, layered as in OOQP: the interior-point recipe is
fixed, the Newton solve is not.
...
rescue through the seam. The flow-planning layer's
...
custom Newton factory --- and accept that it cannot warm-start and owns its
$K$.
...
tolerances live in the adapter, outer controls in
...
improvement of the best residual, the driver stops honestly
...
is about to move anyway, and solving it to $10^{-12}$ buys nothing. The
...
\paragraph{The one-call entry.} For callers who want simple and fast with
no decisions at all:
\begin{lstlisting}[language=C++]
...
\emph{no basin control} --- on problems with multiple equilibria it
converges quickly to whichever equilibrium its trajectory enters;
deliberate equilibrium selection is the alternating chain's job
...
Fischer--Burmeister kink, the semismooth method works \emph{at} the kink.
...
point is the \emph{best visited}, not the last.

\emph{Choose it} for genuinely nonlinear mixed NCPs --- it is the engine
built for exactly that shape --- and expect honest stalls rather than
convergence on nonmonotone problems far from a solution (that is what the
...
The caller binds complete engines into the two \code{StageSolver} seams ---
in the validated configuration, \code{solveVI} with the IPM inner solver
(under the stall cutoff) as globalizer and \code{semismoothNewtonSolve}
(nonmonotone memory 4) as finisher. The chain itself: recomputes the squared
natural residual for every stage result with its \emph{own} fixed functional
(stage solvers' conventions cannot skew the comparison); keeps and finally
returns the best point visited; treats a stage that \emph{throws} as a
stalled stage, not a failure (the round continues with its other stage from
the last good point --- the same attitude PATH takes to subsolver failure);
and, on a round that fails to improve the best residual
(\code{improveFactor}, default 1.0 = any strict improvement continues,
\code{roundsMax} alone bounds cost), restarts the next round from the best
point jiggled componentwise by
$U[-1,1] \cdot \code{perturbScale} \cdot \sqrt{\code{bestMag}}$ --- a
perturbation proportional to the current error, hence vanishing as the chain
closes in, drawn from a fixed-seed generator so runs stay reproducible.
\code{perturbScale = 0} (default) disables perturbation, and stagnation then
ends the chain. The rationale for each element, with the run evidence that
forced it, is the subject of Section~\ref{ch:testing}'s deploy case study
and Chapter~\ref{ch:math}.

\emph{Choose it} when the problem is (or may be) nonmonotone and single
engines return honest stalls: it is bounded, reproducible, honest about
convergence, and the only configuration in this library that has solved the
deployment game.
...
\item \code{levenbergmarquardt.hpp} --- the damping primitives
  ...
  welcome) and \code{levenbergMarquardtUpdate} (the $\lambda$ policy), plus
...
Table~\ref{tab:engine-choice} compresses the catalog into the questions that
actually decide --- and the library also mechanizes it
...
the IPM path) and \code{solveModelAuto} (semismooth first; on an honest
...
\addlinespace
Nonmonotone; single engines stall honestly
...
The flow-planning application lives in its own static library,
...
$\theta = \sum_i P_i \bigl((\mathit{target}_i - R_i)/D_i\bigr)^2$. Note the
recorded gotcha: $\theta$ is a weighted sum of squared \emph{fractions} ---
...
heuristic, which also \emph{calibrates the budget}: an \code{Instance} is
born with \code{tonMileLimit = 0} and the solvers throw on a non-positive
budget, so the standard sequence sets \code{inst.tonMileLimit =
...
algebra is machine-verified (\code{network/doc/ns2-newton-check.mac}).
...
a typo can never silently no-op). \code{applyProfileConfig} consumes the
\code{profile.*} keys (all \code{InstanceProfile} fields);
...
because fidelity lives in its details.
...
\item \textbf{Pair \code{=e=} rows with free multipliers as $H$.} Equality
  ...
  $x$; intermediate GAMS variables defined by equalities may also live here
  when they are provably interior (document the argument --- e.g.\ a
...
\item \textbf{Print the GAMS output parameters.} A report-only check that
  ...
  the GAMS listing by eye before any gate is pinned.
\item \textbf{Gate on verified answers.} Once the author confirms a solution
  matches the reference system, pin the test to it; for games with multiple
  equilibria, gate against a \emph{roster} of verified equilibria and have
  the check print any converged non-roster point as a paste-ready roster
  candidate (Chapter~\ref{ch:testing}).
\end{enumerate}
...
individual engines fail legitimately, gates are expressed over the row
...
Steps 2--3 are where newcomers stumble: the budget calibration is not
optional (an uncalibrated instance throws), and the engine choice changes
the meaning of \code{iterMax}. Everything between reduction and unpacking
--- screens, the LCP build, the certificate loop, nondimensionalization ---
is internal to \code{solveFlowPlan}; the individual stages remain public
(Section~\ref{sec:network}) for tests and custom pipelines.
...
subtlety recorded from experience: the kink indicator lives in the full
...
land-mine for the future Java binding.
% ----------------------------------------------
% Copyright Ben Paul Wise. All Rights Reserved.
% ----------------------------------------------
