...
\code{VIMCP}; all mathematical statements are deferred to
...
\begin{tabular}{ll}
\toprule
\code{include/}, \code{lib/} & the core library \code{vimcp}: types, engines, drivers \\
\code{network/}              & the flow-planning layer \code{vimcpnet} (Section~\ref{sec:network}) \\
...
One header, \code{vimcp.hpp}, defines the domain vocabulary. It is included
...
\code{VIMCP}, so the Eigen dependency stays localized and reversible.
...
\code{vimcpnet} (\code{network/include}, \code{network/lib}, namespace
\code{VIMCP::Network}), layered strictly on top of \code{vimcp}: the core
...
\code{option(VIMCPNET\_BUILD\_GUI)} with a self-disabling subdirectory when
...
The \code{gms/} layer (library \code{vimcpgms}, namespace
\code{VIMCP::Gms}) performs the translation recipe mechanically for the
...
\code{vimcp\_add\_gtest(name)} or \code{vimcpnet\_add\_gtest(name)} ---
