...
| G2 | **scqkp extraction.** Behavior-preserving refactor of `rationTargets`'s continuous quadratic-knapsack core into a shared `scqkp(demand, priority, meetable)` (greedy.hpp), reused per asset column by G4. Existing suite re-run green (58/58) with unchanged values. | S | G1 | done (2026-07-07) |
...
| D6 | **Testing-chapter section on the network GUI.** Explain the viewer's main layout (instance panel, Show Links modes, swaps/purify group, busy bar, rankings lists, status line) with a large figure `doc/images/initial-nw-gui.png` (image present as of 2026-07-07). Prose per `../../doc/style-instructions.md` (standing instruction for all written products, Ben 2026-07-07). SCOPE GREW at Ben's request to a full "Visual validation" section (`sec:t-viewers`, part3.tex after case study 1): network viewer walkthrough against the figure; re-implementation-grade pseudo-code (target: Java 17) for scqkp + greedyPlan, bestSwap + swapToOptimum, and the budget-limited fleetGreedy; the optimizer behind the Optimal overlay named; the tiny-flows explanation (interior point -> analytic center vs edge-following -> forest-sparse vertex) and the purify pass; the fleet viewer with asset/vehicle class definitions. First figure in the report (43 -> 51 pp). | S | D5, F2 | done (2026-07-07; Ben's read pending) |
...
- 2026-07-03: B2 done. `greedy.hpp`/`greedy.cpp`: `rationTargets` (continuous
  quadratic-knapsack `scqkp` with exclude-and-resolve clamp per gate-1 ruling; `lambda <= 0` round means
...
- 2026-07-07: D6 done -- report section 3.5 "Visual validation: the
  ...
  planner (scqkp + phase-2 loop), swap-to-optimum (2-exchange), and
