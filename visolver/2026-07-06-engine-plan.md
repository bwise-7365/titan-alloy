...
CONFIRMED AND COMMITTED (recorded 2026-07-10): the roster gate has run
green in every full-suite run since (the GP-track runs 191/191 through
214/214, commits b16e9a1..8e9edb4). The deploy game stands solved TWICE —
two mirror-twin Nash equilibria on the roster.
...
## Remove unneeded GAMS references (Ben 2026-07-08) — DONE 2026-07-10
...
DONE 2026-07-10, one pass: git renames `gams/` -> `gms/` and
`test/gams_{alloceff,deploy}_test.cpp` -> `test/gms_*`; CMake targets
`gams_alloceff_test`/`gams_deploy_test` -> `gms_*` and aggregate
`run_gams_tests` -> `run_gms_tests` (root, gms/, apps CMakeLists);
comment/doc references updated (mcpengines.hpp, alternating_chain_test,
josephy_newton_test, the two renamed test headers' mapping lines, apps
gms-reader comments, CLAUDE.md, report part2/part3/part4 `\code` names).
KEPT: the disclaimers, genuine GAMS/PATH mentions (reference listings,
`.L` semantics, the trademark standing rule), refs.bib citations, the
corpus `doc/*.gms` / `doc/*.solve.lst` files, and dated historical
