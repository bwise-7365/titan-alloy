...
\input{examples/tab_compete_paired}
...
subset of the columns for every row. This section reports what happens at the size
of a problem an economist actually brings: twenty countries of ten variables each,
so that $A$ is $200$ by $200$ and holds forty thousand cells, with eighty quarters
of data. Enumeration is not merely slow at that size; it is impossible by a margin
that no amount of patience closes. The section says why, describes what replaced it,
...
microseconds each that is $5 \times 10^{17}$ seconds, or seventeen thousand million
...
matrices differ in arrangement alone.
...
\input{examples/tab_alasso_sigma}
